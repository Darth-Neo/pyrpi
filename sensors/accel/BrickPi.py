...
BIT_I2C_SAME = 0x02  #  The transmit data, and the number of bytes to read
                     #  and write isn't going to change. defined for each device.
...
def motorRotateDegree(power, deg, port, sampling_time=.1, delay_when_stopping=.05):
    ...
    for i in range(num_motor):
        ...
        final_val[i] = init_val[i] + (
        deg[i] * 2)  # Final value when the motor has to be stopped;One encoder value counts for 0.5 degrees
        ...
        if not result:
            for i in range(num_motor):  # Do for each of the motors
                ...
                if (deg[i] > 0 and final_val[i] > init_val[i]) or (deg[i] < 0 and final_val[i] < init_val[i]):
                    ...
                    run_stat[i] = 1
                    BrickPi.MotorSpeed[port[i]] = -power[i] if deg[i] > 0 else power[
                        i]  # Run the motors in reverse direction to stop instantly
                    BrickPiUpdateValues()
...
def GetBits(byte_offset, bit_offset, bits):
    ...
    while i:
        result *= 2
        result |= ((Array[(byte_offset + ((bit_offset + Bit_Offset + (i - 1)) / 8))] >>
                    ((bit_offset + Bit_Offset + (i - 1)) % 8)) & 0x01)
        i -= 1
    Bit_Offset += bits
    return result
...
def BrickPiSetupSensors():
    ...
    for i in range(2):
        ...
        for ii in range(2):
            ...
            ########################
            if (Array[BYTE_SENSOR_1_TYPE + ii] == TYPE_SENSOR_I2C or
...
def BrickPiUpdateValues():
    ...
    while i < 2:
        ...
        for ii in range(2):
            ...
            if (BrickPi.SensorType[port] == TYPE_SENSOR_I2C or BrickPi.SensorType[port] == TYPE_SENSOR_I2C_9V or
        ...
        for ii in range(2):
            ...
            elif BrickPi.SensorType[port] == TYPE_SENSOR_I2C or BrickPi.SensorType[port] == TYPE_SENSOR_I2C_9V or \
                            BrickPi.SensorType[port] == TYPE_SENSOR_ULTRASONIC_CONT:
                #######################
