...
class MPL115A2(object):
    ...
    def getPT(self):
        ...
        if self.debug:
            logger.debug(u"Padc: %04i (0x%04X)" % (self.Padc, self.Padc))

        # Getting Tadc
        self.Tadc = (self.Tadc_tmp & 0xFFC0) >> 6
        if self.debug:
            logger.debug(u"Tadc: %04i (0x%04X)" % (self.Tadc, self.Tadc))
        ...
        self.a0 = float(str(self.a0_main) + u"." + str(self.a0_fraction))
        if self.a0_signed:
            ...
            self.a0 = float(str(self.a0_main) + u"." + str(self.a0_fraction))
        if self.debug:
            logger.debug(u"a0: %3.3f" % self.a0)
        ...
        if self.b1_signed:
            ...
            self.b1 = float(str(self.b1_main) + u"." + str(self.b1_fraction))
            self.b1 = -self.b1
        else:
            self.b1_main = (self.b1_temp & 0x6000) >> 13
            self.b1_fraction = (self.b1_temp & 0x1FFF)
            self.b1 = float(str(self.b1_main) + u"." + str(self.b1_fraction))
        if self.debug:
            logger.debug(u"b1: %3.3f" % self.b1)
        ...
        if self.b2_signed:
            ...
            self.b2 = float(str(self.b2_main) + u"." + str(self.b2_fraction))
            self.b2 = -self.b2
        else:
            self.b2_main = (self.b2_temp & 0x4000) >> 14
            self.b2_fraction = (self.b2_temp & 0x3FFF)
        ...
        if self.debug:
            logger.debug(u"b2: %3.3f" % self.b2)
        ...
        if self.c12_signed:
            ...
            self.c12 = float(u"0." + u"%09d" % self.c12_fraction)
            self.c12 = -self.c12
        else:
            self.c12_signed = (self.c12_temp & 0x8000) >> 15
            self.c12_fraction = (self.c12_temp & 0x7FFC) >> 2
        ...
        if self.debug:
            logger.debug(u"c12: %3.3f" % self.c12)
        ...
        if self.debug:
            logger.debug(u"Pcomp: %3.3f" % self.Pcomp)
